...
from roll_handler import process_roll
...
MEMU_X_BUTTON = {'x': 1270,'y': 16}
MEMU_CLOSE_PROMPT_BUTTON = {'x': 620,'y': 430}
MEMU_RESTART_PROMPT_BUTTON = {'x': 700,'y': 430}
MEMU_PROMPT_CANCEL_BUTTON = {'x': 800,'y': 315}

MEMU_MANAGER_ICON = {'x': 1875,'y': 190}                            # YOU NEED TO SET THIS
MEMU_MANAGER_START_STOP = {'x': 430,'y': 60}
MEMU_RELAUNCH_ERROR_CLOSE_BUTTON = {'x': 640,'y': 460}

def wait(given_time):
    time.sleep(TIMING_MULT * given_time)

def touch(x, y):
    pyautogui.click(x=(x + HOR_OFFSET), y=(y + VERT_OFFSET))
...
def click_until(*images):
    proc = multiprocessing.Process(target=_click_until, args=(images))
    proc.start()
...
def _click_until(*images):
    ...
    while True:
        for pos, image in enumerate(images):
            ...
            wait(0.5)                                               # Give people some time to 
                                                                    # close the script.
...
def wait_until(*images):
    proc = multiprocessing.Process(target=_wait_until, args=(images))
    proc.start()
    proc.join(TIME_LIMIT)
...
def restart_memu(refresh_ip = False):

    # Note: this method is slower than just restarting,
    # but on my machine, MEmu occasionally 'stops responding'
    # so I think this solution is more rubust.

    os.system('taskkill /f /t /im  MEmu.exe')

    if refresh_ip and VPN_SUPPORT:
        vpn.refresh_connection()

    touch(**MEMU_MANAGER_ICON)

    while True:
        wait_until('memu_manager_start_button')

        if not (refresh_ip and VPN_SUPPORT):
            time.sleep(30)                                          # If you restart MEmu too quickly,
                                                                    # you encounter session lock error.
        touch(**MEMU_MANAGER_START_STOP)

        result = wait_until('grand_order_icon', 
                            'memu_restart_error')
        if result == 0:                                             # Restart Successful
            clear_data()
            break
        elif result == 1:
            touch(**MEMU_RELAUNCH_ERROR_CLOSE)

def test_image(image):
    for _ in range(20):
        print(image_is_on_screen(image))
    exit()
...
def play_game():
    ...
    wait_until('setup_party_prompt_7_mash')
    wait(3)
    ...
    while True:
        wait_until('summon_screen_close')                           # Changed because new accounts get a ticket.
    ...
    # Tag Rolls
    process_roll(folder_name)

if __name__ == '__main__':

    num_rolls = 0
    was_previously_at_launcher = False                                  # Prevents infinite loops
    ...
    while True:
        try:
            ...
            # Failed To Launch
                ...
                if was_previously_at_launcher:
                    # MEmu is not responding
                    was_previously_at_launcher = False
                    restart_memu()
                else:  
                    was_previously_at_launcher = True
